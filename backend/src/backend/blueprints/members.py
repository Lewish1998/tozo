...
from backend.models.member import insert_member, update_member_email_verified, select_member_by_id, update_member_password, select_member_by_email
...
async def verify_email(data: TokenData) -> ResponseReturnValue:
    ...
    try:
        member_id = serializer.loads(
            data.token, max_age=ONE_MONTH
        )
...
async def reset_password(data: ResetPasswordData) -> ResponseReturnValue:
    ...
    This requires the user to supply a valid token and a new password.
    """
    serializer = URLSafeTimedSerializer(
